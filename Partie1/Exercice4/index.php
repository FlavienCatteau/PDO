        ...
        <main>
            <div class="text-center">
                <?php
                      ...
                      $sql  = 'SELECT `lastName`, `firstName`, `card` FROM clients WHERE card = 1';
                      $customersCard = $bdd->query($sql); // On appelle le nom et le prénom des clients, on limite à 20 (donc les noms des 20 premiers clients)
                      if ($customersCard->execute()){
                          $customersList = $customersCard->fetchAll(PDO::FETCH_ASSOC);
                      }
